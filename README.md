...
| **블로그·인스타·유튜브 링크** | `brand.social` | 푸터 버튼 + 우측 고정 아이콘 |
...
```ts
{
  ...
  field: 'DERMATOLOGY',              // 진료과목 (카드 상단 영문 라벨)
...
> 현재 7건은 **실제 수강생 후기**입니다. 추가 후기가 생기면 같은 형식으로 덧붙이면 됩니다.
> 카드 개수에 제한은 없고, 개수에 맞춰 좌우 슬라이드가 자동으로 늘어납니다.
...
- [x] `brand.social` 블로그 / 인스타그램 / 유튜브 주소
- [ ] 도메인 연결 후 Vercel 환경변수 `NEXT_PUBLIC_SITE_URL` 설정 (8번 참고)
- [x] 교육 문의 전화 (070-8064-4610)
- [x] `brand.company` 주소 · 사업자등록번호 · 이메일
- [x] `reviews` 실제 수강생 후기 7건 반영
